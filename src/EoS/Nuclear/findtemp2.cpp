  ...
  *keyerrt = 0;
  ...
  while(it < itmax) {
    ...
    // lower temperature
    {
      ...
      real fs[4];
      // point 0
      int ifs = 0 + NUC_TABLE_NVAR*(irho-1 + nrho*((itemp-1) + ntemp*(iye-1)));
      fs[0] = alltables[ifs];
      // point 1
      ifs = 0 + NUC_TABLE_NVAR*(irho + nrho*((itemp-1) + ntemp*(iye-1)));
      fs[1] = alltables[ifs];
      // point 2
      ifs = 0 + NUC_TABLE_NVAR*(irho-1 + nrho*((itemp-1) + ntemp*(iye)));
      fs[2] = alltables[ifs];
      // point 3
      ifs = 0 + NUC_TABLE_NVAR*(irho + nrho*((itemp-1) + ntemp*(iye)));
      fs[3] = alltables[ifs];
    ...
    // upper temperature
    {
      ...
      real fs[4];
      // point 0
      int ifs = 0 + NUC_TABLE_NVAR*(irho-1 + nrho*((itemp) + ntemp*(iye-1)));
      fs[0] = alltables[ifs];
      // point 1
      ifs = 0 + NUC_TABLE_NVAR*(irho + nrho*((itemp) + ntemp*(iye-1)));
      fs[1] = alltables[ifs];
      // point 2
      ifs = 0 + NUC_TABLE_NVAR*(irho-1 + nrho*((itemp) + ntemp*(iye)));
      fs[2] = alltables[ifs];
      // point 3
      ifs = 0 + NUC_TABLE_NVAR*(irho + nrho*((itemp) + ntemp*(iye)));
      fs[3] = alltables[ifs];
...
void nuc_eos_C_linterp_for_temp( const real x, const real y, const real z,
  ...
  int iv;
  if ( keymode == 0 ) iv = 1; // energy mode
  if ( keymode == 2 ) iv = 2; // entropy mode
  if ( keymode == 3 ) iv = 0; // pressure mode
...
// prepare
  lt = lt0;
  lt1 = LOG10( MIN ( POW( (real)10.0, ltmax ), ( 1.05 ) * ( POW( (real)10.0, lt0 ) ) ) );
  lt2 = LOG10( MAX ( POW( (real)10.0, ltmin ), ( 0.95 ) * ( POW( (real)10.0, lt0 ) ) ) );
...
// dE/dt > 0, so eps(lt1) > eps(lt2)
  ...
  while(f1*f2 >= 0.0) {
    ...
    lt1 = LOG10( MIN( POW( (real)10.0, ltmax ), ( 1.05 ) * ( POW( (real)10.0, lt1 ) ) ) );
    lt2 = LOG10( MAX( POW( (real)10.0, ltmin ), ( 0.95 ) * ( POW( (real)10.0, lt2 ) ) ) );
    nuc_eos_C_cubinterp_some( lr, lt1, ye, f1a, alltables,
                              nrho, ntemp, nye, nvars, logrho, logtemp, yes);
   ...
   while(f1a < f2a && ifixdeg < ifixdeg_max) {
    ...
    if(bcount >= maxbcount) {
      *keyerrt = 667;
      ...
      // bdry_chk = (f1a[iv] - var0)*(f2a[iv] - var0);
  ...
  for (it=0; it<itmax; it++) {
    dlt = LOG10( POW( (real)10.0, dlt ) * 0.5 );
    ltmid = LOG10( POW( (real)10.0, lt ) + POW( (real)10.0, dlt ) );
    nuc_eos_C_cubinterp_some( lr, ltmid, ye, f2a, alltables, 
                              nrho, ntemp, nye, nvars, logrho, logtemp, yes );
    fmid = f2a[iv] - var0;
